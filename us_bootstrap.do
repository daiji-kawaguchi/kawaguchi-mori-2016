...
cap log close;
set more off;
log using "stataproject\educ_wagegap\log\bootstrap_us.log",replace;
clear all;
set mem 2g;
set seed 1;
local nrep=500;
set matsize 500;
matrix results=J(`nrep', 16, .);

forvalues i=1/`nrep' {;
di `i';
qui clear;
qui use "stataproject\educ_wagegap\data\usregress.dta";
qui bsample;
...
/* second stage; eq.(7) */;
qui reg rel_age_wage trend hdif_sup lagg_ch _Dagegroup4_2-_Dagegroup4_4  [aw=weight];
...
qui matrix results[`i', 16]=_b[trend];

};

svmat results;
centile results1-results16, centile(2.5 50 97.5);
log close;
